import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import { fetchStationWeather } from "../services/weatherService";
import { decodeMetarSummary } from "../utils/metarDecoder";
import { classifyFromMetar } from "../utils/classifyFlightCategory";
import "../styles/modelo-efb.css";
import MarquisaMark from "../components/MarquisaMark";

const SCREENS = [
  { id: "home", label: "Home" },
  { id: "weather", label: "METAR" },
  { id: "dashboard", label: "Briefing" },
  { id: "login", label: "Login" },
  { id: "exams", label: "Simulados" },
  { id: "billing", label: "PRO" },
];

const QUICK = ["SBGR", "SBRJ", "SBSP", "SBCF", "SBGL"];

function toneOf(cat) {
  const c = String(cat || "").toUpperCase();
  if (c === "VFR") return "vfr";
  if (c === "MVFR") return "mvfr";
  if (c === "IFR" || c === "LIFR") return "ifr";
  return "";
}

function plainOf(cat) {
  const c = String(cat || "").toUpperCase();
  if (c === "VFR") return "Condições visuais boas";
  if (c === "MVFR") return "Visual com restrições";
  if (c === "IFR" || c === "LIFR") return "Condições instrumentais";
  return "Aguardando boletim";
}

function Shell({ utc, actions, children }) {
  return (
    <div className="mi-shell">
      <header className="mi-top">
        <div className="mi-brand">
          <div className="mi-mark" aria-hidden="true">
            <MarquisaMark size={20} />
          </div>
          <div>
            <h1>MARQUISA</h1>
            <p>EFB · bagagem eletrônica</p>
          </div>
        </div>
        <div className="mi-clock">
          {utc}
          <span>UTC · Zulu</span>
        </div>
        <div className="mi-actions">{actions}</div>
      </header>
      <div className="mi-stage">{children}</div>
      <footer className="mi-footer">
        <span>Painel · METAR · Briefing · Simulados</span>
        <span>Voe seguro · Voe preparado</span>
      </footer>
    </div>
  );
}

function WxPanel({ icao, setIcao, onLoad, loading, error, metar, taf, airportName, category, summary }) {
  const tone = toneOf(category);
  const catLabel =
    category === "NO_DATA" || category === "UNKNOWN" || !category ? "—" : category;

  return (
    <div className="mi-card">
      <div className="mi-wx-head">
        <div>
          <div className="mi-icao">{icao || "----"}</div>
          <span className="mi-airport">{airportName || "Aeródromo"}</span>
        </div>
        <div className={`mi-cat${tone ? ` mi-cat--${tone}` : ""}`}>
          <strong>{catLabel}</strong>
          <span>{plainOf(category)}</span>
        </div>
      </div>
      {summary?.hints?.length ? (
        <ul className="mi-hints">
          {summary.hints.map((h) => (
            <li key={h}>{h}</li>
          ))}
        </ul>
      ) : (
        <p className="mi-empty">{loading ? "Carregando…" : "Digite um ICAO e consulte."}</p>
      )}
      {error ? <div className="mi-error">{error}</div> : null}
      <div className="mi-raw">
        <header>METAR</header>
        <pre>{metar || "—"}</pre>
      </div>
      <div className="mi-raw">
        <header>TAF</header>
        <pre>{taf || "—"}</pre>
      </div>
      <form
        className="mi-form"
        onSubmit={(e) => {
          e.preventDefault();
          onLoad(icao);
        }}
      >
        <label className="mi-field">
          <span>ICAO</span>
          <input
            value={icao}
            onChange={(e) => setIcao(e.target.value.toUpperCase().slice(0, 4))}
            maxLength={4}
            placeholder="SBGR"
          />
        </label>
        <button type="submit" className="mi-btn mi-btn--primary" disabled={loading}>
          {loading ? "…" : "Consultar"}
        </button>
      </form>
      <div className="mi-quick">
        {QUICK.map((code) => (
          <button
            key={code}
            type="button"
            className={icao === code ? "is-active" : ""}
            onClick={() => {
              setIcao(code);
              onLoad(code);
            }}
          >
            {code}
          </button>
        ))}
      </div>
    </div>
  );
}

function HomeScreen({ wx }) {
  return (
    <>
      <section className="mi-hero">
        <div className="mi-card">
          <span className="mi-kicker">Marquisa</span>
          <h2 className="mi-title">Bagagem de voo digital</h2>
          <p className="mi-lead">
            METAR e TAF sem cadastro. Briefing e simulados com conta. Layout denso,
            sem enfeite — estilo EFB.
          </p>
          <div className="mi-alert">Amarelo só para alerta · azul para ação</div>
          <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            <button type="button" className="mi-btn mi-btn--primary">
              Consultar METAR
            </button>
            <button type="button" className="mi-btn mi-btn--ghost">
              Criar conta
            </button>
          </div>
        </div>
        <WxPanel {...wx} />
      </section>
      <div className="mi-grid-3">
        <article className="mi-card mi-module">
          <span className="mi-tag">Livre</span>
          <h3>METAR / TAF</h3>
          <p>Boletim por ICAO + categoria VFR/MVFR/IFR.</p>
        </article>
        <article className="mi-card mi-module">
          <span className="mi-tag mi-tag--lock">Conta</span>
          <h3>Briefing A–B–C</h3>
          <p>Origem, destino e alternativa.</p>
        </article>
        <article className="mi-card mi-module">
          <span className="mi-tag mi-tag--lock">Conta</span>
          <h3>Simulados ANAC</h3>
          <p>PP, Comissário e trilha PRO.</p>
        </article>
      </div>
    </>
  );
}

function WeatherScreen({ wx }) {
  return (
    <div className="mi-grid-2">
      <div className="mi-card">
        <span className="mi-kicker">Consulta pública</span>
        <h2 className="mi-title">METAR e TAF</h2>
        <p className="mi-lead">Sem cadastro. ICAO de 4 letras.</p>
        <form
          className="mi-form"
          onSubmit={(e) => {
            e.preventDefault();
            wx.onLoad(wx.icao);
          }}
        >
          <label className="mi-field">
            <span>ICAO</span>
            <input
              value={wx.icao}
              onChange={(e) => wx.setIcao(e.target.value.toUpperCase().slice(0, 4))}
              maxLength={4}
            />
          </label>
          <button type="submit" className="mi-btn mi-btn--primary" disabled={wx.loading}>
            Consultar
          </button>
        </form>
        <div className="mi-quick">
          {QUICK.map((code) => (
            <button
              key={code}
              type="button"
              className={wx.icao === code ? "is-active" : ""}
              onClick={() => {
                wx.setIcao(code);
                wx.onLoad(code);
              }}
            >
              {code}
            </button>
          ))}
        </div>
      </div>
      <WxPanel {...wx} />
    </div>
  );
}

function DashboardScreen() {
  return (
    <>
      <div className="mi-card">
        <span className="mi-kicker">Briefing</span>
        <h2 className="mi-title">Rota A–B–C</h2>
        <form className="mi-route" onSubmit={(e) => e.preventDefault()}>
          <label className="mi-field">
            <span>A · Origem</span>
            <input defaultValue="SBCF" maxLength={4} />
          </label>
          <label className="mi-field">
            <span>B · Destino</span>
            <input defaultValue="SBGR" maxLength={4} />
          </label>
          <label className="mi-field">
            <span>C · Alternativa</span>
            <input defaultValue="SBRJ" maxLength={4} />
          </label>
          <button type="submit" className="mi-btn mi-btn--primary">
            Gerar
          </button>
        </form>
      </div>
      <div className="mi-grid-3">
        <article className="mi-card mi-module">
          <span className="mi-tag">VFR</span>
          <h3>SBCF</h3>
          <p>Origem · Tancredo Neves</p>
        </article>
        <article className="mi-card mi-module">
          <span className="mi-tag">VFR</span>
          <h3>SBGR</h3>
          <p>Destino · Guarulhos</p>
        </article>
        <article className="mi-card mi-module">
          <span className="mi-tag">VFR</span>
          <h3>SBRJ</h3>
          <p>Alternativa · Santos Dumont</p>
        </article>
      </div>
    </>
  );
}

function LoginScreen() {
  return (
    <div className="mi-auth">
      <div className="mi-card">
        <span className="mi-kicker">Acesso</span>
        <h2 className="mi-title">Entrar</h2>
        <p className="mi-lead">Conta libera briefing e simulados.</p>
        <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
          <label className="mi-field">
            <span>E-mail</span>
            <input type="email" placeholder="voce@email.com" style={{ letterSpacing: "0.02em", fontWeight: 500 }} />
          </label>
          <label className="mi-field">
            <span>Senha</span>
            <input type="password" placeholder="••••••••" />
          </label>
          <button type="button" className="mi-btn mi-btn--primary">
            Entrar
          </button>
          <button type="button" className="mi-btn mi-btn--ghost">
            Continuar com Google
          </button>
        </div>
      </div>
    </div>
  );
}

function ExamsScreen() {
  return (
    <div className="mi-grid-2">
      <div className="mi-card">
        <span className="mi-kicker">Simulado</span>
        <h2 className="mi-title">PP-A · Q12</h2>
        <p className="mi-lead">Em relação às regras de voo VFR, assinale a correta.</p>
        <button type="button" className="mi-option is-selected">
          A) Manter referência visual com o solo
        </button>
        <button type="button" className="mi-option">
          B) Dispensar comunicação em espaços controlados
        </button>
        <button type="button" className="mi-option">
          C) Operar sem teto mínimo definido
        </button>
        <button type="button" className="mi-option">
          D) Ignorar restrições de visibilidade
        </button>
      </div>
      <div className="mi-card">
        <span className="mi-kicker">Cursos</span>
        <ul className="mi-list">
          <li>PP-A — conta free</li>
          <li>CMS Comissário — conta free</li>
          <li>PC / IFR — PRO</li>
        </ul>
        <button type="button" className="mi-btn mi-btn--primary" style={{ marginTop: 12 }}>
          Ver PRO
        </button>
      </div>
    </div>
  );
}

function BillingScreen() {
  return (
    <div className="mi-grid-2">
      <div className="mi-card">
        <span className="mi-kicker">Assinatura</span>
        <h2 className="mi-title">Marquisa PRO</h2>
        <div className="mi-price">
          R$ 19,90 <small>/ mês</small>
        </div>
        <ul className="mi-list">
          <li>Simulados ilimitados</li>
          <li>Todas as matérias</li>
          <li>Briefings na nuvem</li>
        </ul>
        <button type="button" className="mi-btn mi-btn--primary" style={{ marginTop: 12 }}>
          Assinar PRO
        </button>
      </div>
      <div className="mi-card">
        <span className="mi-kicker">Conta free</span>
        <ul className="mi-list">
          <li>METAR/TAF público</li>
          <li>Briefing e ferramentas</li>
          <li>Prova PP/CMS</li>
        </ul>
      </div>
    </div>
  );
}

export default function ModeloLabI() {
  const [screen, setScreen] = useState("home");
  const [utc, setUtc] = useState("--:--:--Z");
  const [icao, setIcao] = useState("SBGR");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [metar, setMetar] = useState("");
  const [taf, setTaf] = useState("");
  const [airportName, setAirportName] = useState("");

  useEffect(() => {
    const tick = () => {
      const now = new Date();
      const hh = String(now.getUTCHours()).padStart(2, "0");
      const mm = String(now.getUTCMinutes()).padStart(2, "0");
      const ss = String(now.getUTCSeconds()).padStart(2, "0");
      setUtc(`${hh}:${mm}:${ss}Z`);
    };
    tick();
    const id = setInterval(tick, 1000);
    return () => clearInterval(id);
  }, []);

  async function load(code) {
    const clean = String(code || "").trim().toUpperCase();
    if (clean.length !== 4) {
      setError("Informe um ICAO de 4 letras.");
      return;
    }
    setIcao(clean);
    setLoading(true);
    setError("");
    try {
      const station = await fetchStationWeather(clean);
      setMetar(station.metar || "");
      setTaf(station.taf || "");
      setAirportName(station.airport?.name || station.airport?.city || clean);
    } catch (e) {
      setMetar("");
      setTaf("");
      setAirportName("");
      setError(e?.message || "Falha ao consultar.");
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => {
    load("SBGR");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const category = classifyFromMetar(metar);
  const summary = decodeMetarSummary(metar, "pt-BR");
  const wx = {
    icao,
    setIcao,
    onLoad: load,
    loading,
    error,
    metar,
    taf,
    airportName,
    category,
    summary,
  };

  const actions =
    screen === "home" || screen === "login" || screen === "weather" ? (
      <>
        <button type="button" className="mi-btn mi-btn--ghost">
          Entrar
        </button>
        <button type="button" className="mi-btn mi-btn--primary">
          Criar conta
        </button>
      </>
    ) : (
      <>
        <button type="button" className="mi-btn mi-btn--ghost">
          Perfil
        </button>
        <button type="button" className="mi-btn mi-btn--primary">
          PRO
        </button>
      </>
    );

  const body = {
    home: <HomeScreen wx={wx} />,
    weather: <WeatherScreen wx={wx} />,
    dashboard: <DashboardScreen />,
    login: <LoginScreen />,
    exams: <ExamsScreen />,
    billing: <BillingScreen />,
  }[screen];

  return (
    <div className="mi-lab">
      <div className="mi-toolbar">
        <strong>Modelo I · EFB</strong>
        {SCREENS.map((item) => (
          <button
            key={item.id}
            type="button"
            className={`mi-chip${screen === item.id ? " is-active" : ""}`}
            onClick={() => setScreen(item.id)}
          >
            {item.label}
          </button>
        ))}
        <div className="mi-links">
          <Link to="/">Site</Link>
        </div>
      </div>
      <Shell utc={utc} actions={actions}>
        {body}
      </Shell>
    </div>
  );
}
